...
import top.chenfu.demo.dao.MenuMapper;
    ...
    public Example createExample(Menu menu) {
        ...
        if (menu != null) {
            ...
            if (!StringUtils.isEmpty(menu.getUri())) {
                criteria.andEqualTo("uri", menu.getUri());
            }
            // 菜单名称
            ...
            // 菜单等级-0，1，2分别对应三级菜单
            if (!StringUtils.isEmpty(menu.getMenuLevel())) {
                criteria.andEqualTo("menuLevel", menu.getMenuLevel());
            }
            // 注释
            if (!StringUtils.isEmpty(menu.getComment())) {
                criteria.andEqualTo("comment", menu.getComment());
            }
            // 父目录
